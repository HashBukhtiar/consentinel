...
local MARGIN = PATCH_W // 20
local CX, CY = PATCH_X + MARGIN, PATCH_Y + MARGIN
local CW, CH = PATCH_W - 2 * MARGIN, PATCH_H - 2 * MARGIN

local D_N    = 3
local D_GAP  = CW // 24
local D_W    = (CW - (D_N - 1) * D_GAP) // D_N
local D_H    = CH
local D_T    = D_W // 4
local D_HALF = (D_H - 3 * D_T) // 2
local D_X    = CX + (CW - (D_N * D_W + (D_N - 1) * D_GAP)) // 2
local D_Y    = CY

local BASE_IN, BASE_OUT = 0x00FF00, 0xFF0000
